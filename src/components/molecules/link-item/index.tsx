...
const LinkItem: React.FC<LinkItemProperties> = ({ url, css, icon, platformName, ...rest }) => (
  <LinkBox rel={'noopener noreferrer'} href={url} target={'_blank'} css={css} onClick={() => trackEventToUmami({ eventType: 'link', eventValue: `About: Click Social Link - ${platformName} | ${url}`})} {...rest}>
    <InlineIcon
      icon={icon}
